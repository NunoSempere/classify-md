	...
	if err != nil {
		...
		if errors.As(err, &e) {
			...
			case 429:
				...
				log.Printf("Rate limited or exceeded quota")
			case 500:
...
func ClassifySection(section string, topicNames []string, token string) (int, string, error) {
	// Build the list of topics for the prompt
	topicList := ""
	for i, name := range topicNames {
		topicList += fmt.Sprintf("%d. %s\n", i, name)
	}

	prompt := `You are a classification API that categorizes text sections into topics.

Given the following list of topics:
` + topicList + `
And the following text section:
<SECTION>
` + section + `
</SECTION>

Classify this section into the most appropriate topic. Return a JSON object with:
- "reasoning": A brief explanation of why this topic was chosen
- "topic_index": The index number (0-based) of the chosen topic from the list above
- "error": null if successful, or an error message string if classification failed

Choose the single most relevant topic based on the content of the section.`

	var classificationBox ClassificationBox
	schema, err := jsonschema.GenerateSchemaForType(classificationBox)
	if err != nil {
		return -1, "", fmt.Errorf("GenerateSchemaForType error: %v", err)
	}
	openaiSchema := openai.ChatCompletionResponseFormatJSONSchema{
		Name:   "Classification",
		Schema: schema,
		Strict: true,
	}
	...
	// Check for error field - be permissive about what counts as "no error"
	if classificationBox.Error != nil {
		errVal := strings.TrimSpace(*classificationBox.Error)
		// Treat empty, "null", "none", or punctuation-only as no error
		if errVal != "" && errVal != "null" && errVal != "none" && errVal != "nil" {
			// Check if it's just punctuation (like "," or ".")
			isJustPunctuation := true
			for _, r := range errVal {
			...
			if !isJustPunctuation {
				return -1, "", fmt.Errorf("classification error: %s", errVal)
			}
		}
	}

	if classificationBox.TopicIndex < 0 || classificationBox.TopicIndex >= len(topicNames) {
		return -1, "", fmt.Errorf("invalid topic index %d (must be 0-%d)", classificationBox.TopicIndex, len(topicNames)-1)
	}

	return classificationBox.TopicIndex, classificationBox.Reasoning, nil
}
